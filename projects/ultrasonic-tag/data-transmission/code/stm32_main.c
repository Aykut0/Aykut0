...
int16_t adc1_v;
int16_t adc2_v;
uint32_t spiStartCycle = 0;
uint32_t spiEndCycle = 0;
...
// Get current microseconds from DWT
...
void sendDataViaSPI(void)
{
  ...
  // Show the extracted and converted values
  printf("Input data sample: ADC1=%d, ADC2=%d\r\n",
         (int)((adcBuf[0] & 0x0000FFFF) * 3300 / 255),
         (int)((adcBuf[0] >> 16) * 3300 / 255));

  // Process data into byte buffer
  for(uint32_t i = 0; i < ADC_BUF_SIZE; i++)
  {
    // Extract and convert ADC values
    adc1_v = (((int16_t)(adcBuf[i] & 0x0000FFFF))) * 3300 / 255;
    ...
    spiTxBuf[i*2+1] = (uint16_t)adc2_v;
  }
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  DWT_Init();
  // Set SPI CS pin high initially (inactive)
  ...
  printf("STM32 SPI Data Sender Initialized\r\n");
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    if (adcFlag && spiTxComplete)
    {
      adcFlag = RESET;

      // Send ADC data via SPI
      sendDataViaSPI();
      ...
      // Debug print some samples to verify data
      printf("Sample data: ADC1=%d, ADC2=%d\r\n",
             spiTxBuf[0], spiTxBuf[1]);

      // Restart ADC for next batch of samples
      HAL_ADCEx_MultiModeStart_DMA(&hadc1, adcBuf, ADC_BUF_SIZE);
    }
  }
  /* USER CODE END 3 */
...
// UART callback is kept for compatibility but no longer used for ADC restart
